...
export const reg_controler = async (req, res) => {
  try {
    ...
    let dbcreate = await registerModel.create({
      username,
      mobile,
      mail,
      confirmpassword: hased,
    });
    ...
    res
      .status(201)
      .json({ message: "data added succesfully", dbcreate, getJwtToken });
  } catch (error) {
    console.log("something error in register :", error.message);
    if (error.code === 11000)
      return res
        .status(409)
        .json({ msg: "already Registered This username/number" });
  }
};
export const login = async (req, res) => {
  try {
    ...
    if (!mobile || !password)
      return res.status(401).json({ msg: "must be enter value!" });
